...
import elements.Cultist;
import elements.Enemy;
import elements.GoblinBeast;
import elements.Player;
import elements.Solid;
import elements.Sorceress;
...
public class BossStage extends GenericStage{
	...
	public BossStage(Demo game, String direccion, GameScreen mapa, int casillaActualX, int casillaActualY) {
		...
		switch (direccion) {
			...
			case "bU":
				map=ResourceManager.getMap("assets/maps/bossU.tmx");
				break;
			case "bD":
				map=ResourceManager.getMap("assets/maps/bossD.tmx");
				break;
			case "bL":
				map=ResourceManager.getMap("assets/maps/bossL.tmx");
				break;
			case "bR":
				map=ResourceManager.getMap("assets/maps/bossR.tmx");
				break;
		...
		if (!(completada && casillaInicial)) {
			for(MapObject ene:getEnemyList()) {
				...
				switch(props.get("enemigos").toString()) {
					case "boss":
						Enemy nuevoEnemigo = null;
						switch(Parametros.nivel) {
							case 1:
								nuevoEnemigo=new GoblinBeast((float)props.get("x"), (float)props.get("y"), mainStage, this, mapa, casillaActualX, casillaActualY);
								break;
							case 2:
								nuevoEnemigo=new Cultist((float)props.get("x"), (float)props.get("y"), mainStage, this, mapa, casillaActualX, casillaActualY);
								break;
							case 3:
								nuevoEnemigo=new Sorceress((float)props.get("x"), (float)props.get("y"), mainStage, this, mapa, casillaActualX, casillaActualY);
								break;
						}
						enemigos.add(nuevoEnemigo);
						break;
					case "trampilla":
						trampilla = new Trampilla((float)props.get("x"), (float)props.get("y"), mainStage, this, mapa, casillaActualX, casillaActualY, game);
						break;
		...
		Corazon corazon = new Corazon(trampilla.getX()+5, 100, mainStage, mapa, casillaActualX, casillaActualY);
	...
	public ArrayList<MapObject> getEnemyList(){
		...
		for(MapLayer layer: map.getLayers()) {
			for(MapObject obj: layer.getObjects()) {
				...
				if(defaultProps.containsKey("enemigos")) {
					list.add(obj);	
				}
				if(props.containsKey("enemigos")) {
					list.add(obj);	
				}
